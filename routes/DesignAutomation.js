...
String.prototype.toBase64 = function () {return new Buffer(this).toString('base64');};
...
router.post('/forge/designautomation/workitems', multer({
    ...
    try {
        ...
        let uploadres = await new ForgeAPI.ObjectsApi().copyTo(bucketKey, inputFileNameOSS, outputFileNameOSS, req.oauth_client, req.oauth_token);
        outputurn = uploadres.body.objectId.toBase64();
...
router.post('/forge/callback/designautomation', async /*OnCallback*/ (req, res) => {
    ...
    try {
        let internaltoken = await getInternalToken();
        let workflowid = new Date().toISOString().replace(/[-T:\.Z]/gm, '').substring(0, 14)+'da';
        var options = {
            "method": "POST",
            "hostname": "developer.api.autodesk.com",
            "port": 443,
            "path": "/webhooks/v1/systems/derivative/events/extraction.finished/hooks",
            "headers": {
          ...
          var webhookreq = http.request(options, function (res) {
            translateFile(workflowid, internaltoken);
          });
          
          webhookreq.write(JSON.stringify({
            callbackUrl: `${config.credentials.webhook_url}/api/forge/callback/translationfinished`,
...
async function translateFile(workflowid, internaltoken) {
...
router.post('/forge/callback/translationfinished', async /*OnCallback*/ (req, res) => {
    res.status(200).end();
    const socketIO = require('../server').io;
    socketIO.emit('show', outputurn);
});
/// <summary>
